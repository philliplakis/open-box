...
const workspacePath = mkdtempSync(join(tmpdir(), "openbox-agent-smoke-"))
...
const name = `agent-smoke-${Date.now()}`
const workspace = JSON.parse(command(["workspace", "add", workspacePath, "--name", name])) as { id: string }
...
try {
  ...
  let input: unknown[] = [{
    role: "user",
    content: "The Bun project in /workspace has a failing test. Use the shell to inspect the files, fix the bug, and rerun bun test. Do not change package.json.",
  }]
